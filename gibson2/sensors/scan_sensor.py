...
class ScanSensor(BaseSensor):
    ...
    def __init__(self, env, modalities):
        ...
        self.laser_pose = [env.robots[robot_id].parts[self.laser_link_name].get_pose() for robot_id in range(self.num_robots)]
        self.base_pose = [env.robots[robot_id].parts['base_link'].get_pose() for robot_id in range(self.num_robots)]
    ...
    def get_local_occupancy_grid(self, scan, robot_id):
        """
        ...
        laser_translation = self.laser_pose[robot_id][:3]
        laser_rotation = quat2mat(
            [self.laser_pose[robot_id][6], self.laser_pose[robot_id][3], self.laser_pose[robot_id][4], self.laser_pose[robot_id][5]])
        scan_world = laser_rotation.dot(scan_laser.T).T + laser_translation

        base_translation = self.base_pose[robot_id][:3]
        base_rotation = quat2mat(
            [self.base_pose[robot_id][6], self.base_pose[robot_id][3], self.base_pose[robot_id][4], self.base_pose[robot_id][5]])
    ...
    def get_obs(self, env):
        ...
        for robot_id in range(self.num_robots):
            if self.laser_link_name not in env.robots[robot_id].parts:
            ...
            laser_pose = env.robots[robot_id].parts[self.laser_link_name].get_pose()
            angle = np.arange(
            ...
            state['scan'] = scan
            if 'occupancy_grid' in self.modalities:
                state['occupancy_grid'] = self.get_local_occupancy_grid(scan, robot_id)
